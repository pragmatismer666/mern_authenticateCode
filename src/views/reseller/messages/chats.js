...
export default class Chats extends Component {
    ...
    getMessages(ticket_id) {
        axios.get(config.api.url + '/reseller/message/list/' + ticket_id)
            .then((res) => {
                if (res.status === 200) {
    ...
    setRead(id, is_opener) {
        ...
        axios.post(config.api.url + '/reseller/ticket/setread/' + id, req)
            .then((res) => {
                if (res.status === 200) {
    ...
    handleChatInputPress = e => {
        if (e.key === "Enter") {
            if (this.state.messageInput.length > 0) {
                ...
                axios.post(config.api.url + '/reseller/message/add/', message)
            .then((res) => {
                if (res.status === 200) {
