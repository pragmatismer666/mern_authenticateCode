...
export default class BuyCode extends Component {
    ...
    getProducts() {
        ...
        axios.get(config.api.url + '/mainreseller/product/m/list/' + mainreseller_id)
            .then((res) => {
                if (res.status === 200) {
    ...
    getCredit() {
        ...
        axios.get(config.api.url + '/mainreseller/getcredit/' + user_id)
            .then((res) => {
                if (res.status === 200) {
    ...
    buyCode(id, credit_price) {
        if (document.getElementById(id).value) {
            axios.get(config.api.url + '/mainreseller/countcode/' + id)
                .then((res) => {
                    if (parseInt(res.data) >= parseInt(document.getElementById(id).value)) {
                        ...
                        if (values.amount > 0) {
                            values.credits = Number((this.state.user_credit - parseFloat(credit_price) * values.amount).toFixed(2));
                            if (values.credits >= 0) {
                                this.setState({
                                    values: values
                                });
    ...
    purchase() {
        axios.post(config.api.url + '/mainreseller/purchase/', this.state.values)
            .then((res) => {
                if (res.status === 200) {
