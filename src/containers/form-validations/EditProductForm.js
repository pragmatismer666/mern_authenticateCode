...
class EditProductForm extends Component {
  constructor(props) {
    ...
    this.getCategorys();
    this.getProduct(props.productid);
  ...
  getProduct(_id) {
    axios.get(config.api.url + '/admin/product/list/' + _id)
      .then((res) => {
        if (res.status === 200) {
  ...
  handleSubmit(event, errors, values) {
    if (errors.length === 0) {
      if (this.state.file == null) {
        ...
        axios.post(config.api.url + '/admin/product/edit/' + this.state.product._id, formData, configHeader)
          .then((res) => {
            if (res.status === 200) {
            ...
            <Colxx xs="8" md="8" className="mb-3">
              ...
              <AvForm
                ...
                <AvGroup>
                  ...
                  <AvInput name="product_name" value={this.state.product.product_name} required />
                  <AvFeedback><IntlMessages id="form.reqproduct" /></AvFeedback>
                </AvGroup>
                <AvGroup>
                  <Label><IntlMessages id="form.credit" /></Label>
                  <AvInput name="credit_price" value={this.state.product.credit_price} required />
