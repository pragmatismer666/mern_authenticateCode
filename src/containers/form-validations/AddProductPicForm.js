  ...
  handleSubmit(event, errors, values) {
    if (errors.length === 0) {
      if (this.state.file == null) {
        ...
        axios.post(config.api.url + '/admin/productpic/add', formData, configHeader)
          .then((res) => {
            if (res.status === 200) {
        ...
        <CardBody>
          <Row>
            <Colxx xs="6" md="6" className="mb-3">
              <h6 className="mb-4"><IntlMessages id="form.addnewproductpic" /></h6>
              <AvForm
                ...
                <InputGroup className="mb-3">
                  <CustomInput
                    ...
                    accept="image/*"
                    required
                    errorMessage="Please upload product picture!"
