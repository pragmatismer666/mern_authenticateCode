...
class OpenTicketResellerForm extends Component {
    constructor(props) {
        super();
        this.state = {
            receivers: []
    ...
    getReceivers() {
        axios.get(config.api.url + '/reseller/user/' + localStorage.getItem("user_id"))
            .then((user) => {
                if (user.status === 200) {
                    axios.get(config.api.url + '/reseller/receivers/list/' + user.data[0].main_reseller_id)
                        .then((res) => {
                            if (res.status === 200) {
    ...
    handleSubmit(event, errors, values) {
        ...
        if (errors.length === 0) {
            ...
            axios.post(config.api.url + '/reseller/ticket/add', ticket)
                .then((res) => {
                    if (res.status === 200) {
                        message.ticket_id = res.data._id;
                        axios.post(config.api.url + '/reseller/message/addone', message)
                            .then((res) => {
                                if (res.status === 200) {
                                    refreshTickets();
                                }
...
export default OpenTicketResellerForm;
