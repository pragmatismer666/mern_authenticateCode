...
class OpenTicketMainresellerForm extends Component {
    constructor(props) {
        super();
        this.state = {
            receivers: []
    ...
    getReceivers() {
        axios.get(config.api.url + '/mainreseller/receivers/list/' + localStorage.getItem("user_id"))
        .then((res) => {
            if (res.status === 200) {
    ...
    handleSubmit(event, errors, values) {
        ...
        if (errors.length === 0) {
            ...
            axios.post(config.api.url + '/mainreseller/ticket/add', ticket)
                .then((res) => {
                    if (res.status === 200) {
                        message.ticket_id = res.data._id;
                        axios.post(config.api.url + '/mainreseller/message/addone', message)
                            .then((res) => {
                                if (res.status === 200) {
                                    refreshTickets();
                                }
...
export default OpenTicketMainresellerForm;
