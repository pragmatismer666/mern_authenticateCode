...
//Main Reseller 
router.get('/reseller/:id', (req, res, next) => {
    User.find({ _id: req.params.id })
        .then(data => res.json(data))
        .catch(next)
});

router.get('/reseller/list/:main_reseller_id', (req, res, next) => {
    User.find({ role: 'reseller', main_reseller_id: req.params.main_reseller_id })
        .then(data => res.json(data))
        .catch(next)
});

router.post('/reseller/add', (req, res, next) => {
    if (req.body) {
...
router.post('/reseller/edit/:id', (req, res, next) => {
    User.findOneAndUpdate({ "_id": req.params.id }, req.body)
        .then(data => res.json(data))
        .catch(next)
})

router.post('/reseller/delete/:id', (req, res, next) => {
...
router.get('/specprice/m/list/:id', (req, res, next) => {
    Specprice.find({ "mainreseller_id": req.params.id })
        .then(specprices => {
            let sprices = [];
            sprices = specprices;
            let resSpecPrices = [];
...
//Receiver List
router.get('/receivers/list/:user_id', (req, res, next) => {
    User.find({ "role": "admin" })
        .then(admin => {
            User.find({ "main_reseller_id": req.params.user_id, "role": "reseller" })
                .then(resellers => {
                    let response = [];
                    response = admin.concat(resellers)
                    res.json(response)
                })
                .catch(next)
        })
...
router.get('/product/m/list/:mainreseller_id', (req, res, next) => {
    Product.find({})
        .then(products => {
            let resProducts = [];
            for (let i = 0; i < products.length; i++) {
                ...
                Specprice.findOne({ "product_id": products[i]._id, "user_id": req.params.mainreseller_id })
                    .then(specprice => {
                        if (specprice !== null) {
...
router.get('/info/:id', (req, res, next) => {
    ...
    User.findOne({ "_id": req.params.id })
        .then((data) => {
            credits = data.credit;
            User.countDocuments({ "role": "reseller", "main_reseller_id": req.params.id }, function (err, resellerCount) {
...
router.post('/changecredit/:id', (req, res, next) => {
    User.findOneAndUpdate({ "_id": req.params.id }, { "credit": req.body.credit })
        .then(reseller => {
            User.findOne({ "_id": reseller.main_reseller_id })
                .then(mainreseller => {
                    User.findByIdAndUpdate({ "_id": mainreseller._id }, { "credit": mainreseller.credit - parseInt(req.body.changedCredit) })
                        .then(data => {
                            res.json(data);
                        })
                        .catch(next)
                    // res.json(mainreseller)
                })
                .catch(next)
        })
        .catch(next)
...
//Change Credit
router.post('/m/changecredit/:id', (req, res, next) => {
    User.findByIdAndUpdate({ "_id": req.params.id }, { "credit": req.body.changedCredit })
        .then(data => {
            res.json(data);
        })
        .catch(next)
